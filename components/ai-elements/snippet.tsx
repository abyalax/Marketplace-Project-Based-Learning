...
export const Snippet = ({ code, className, children, ...props }: SnippetProps) => {
  const memoizeCode = useMemo(() => ({ code }), [code]);
  return (
    <SnippetContext.Provider value={memoizeCode}>
...
export const SnippetCopyButton = ({ onCopy, onError, timeout = 2000, children, className, ...props }: SnippetCopyButtonProps) => {
  ...
  return (
    <InputGroupButton aria-label="Copy" className={className} onClick={copyToClipboard} size="sm" title="Copy" {...props}>
